...
TEE_Result shdr_verify_signature(const struct shdr *shdr)
{
	...
	case TEE_MAIN_ALGO_ECDSA:
		...
		/*
		 * crypto_acipher_ecc_verify() isn't instrumented for fault
		 * mitigation so it can't be called as a linked call with
		 * FTMN_CALL_FUNC(), the result is recorded directly
		 * instead.
		 */
		res = crypto_acipher_ecc_verify(shdr->algo, &ecc_key,
						SHDR_GET_HASH(shdr),
						shdr->hash_size,
						SHDR_GET_SIG(shdr),
						shdr->sig_size);
				  ...
				  const struct shdr *shdr)
{
	...
	case TEE_MAIN_ALGO_ECDSA:
		/* Not a linked call, see comment in shdr_verify_signature() */
		res = crypto_acipher_ecc_verify(shdr->algo, key->pub_key.ecc,
						SHDR_GET_HASH(shdr),
						shdr->hash_size,
						SHDR_GET_SIG(shdr),
						shdr->sig_size);
